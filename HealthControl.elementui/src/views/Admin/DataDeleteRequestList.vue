<template>
  <div>
    ...
    <PaginationTable
      ...
      url="/TDataDeleteRequest/List"
      :column="columnList"
      :where="where"
    >
      <template v-slot:header>
        <ExportButton exportUrl="/TDataDeleteRequest/Export" :where="searchForm" />
...
const BatchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(`确认删除选中的 ${ids.length} 条删除申请记录？该操作仅删除申请记录本身，不会再次执行数据删除。`, "批量删除", {
      confirmButtonText: "确认删除",
      cancelButtonText: "取消",
      type: "warning",
    });
    const { Success } = await Post("/TDataDeleteRequest/BatchDelete", { Ids: ids });
    if (Success !== false) {
...
const audit = async (id, action, remark) => {
  try {
    await Post("/TDataDeleteRequest/Audit", {
      Id: id,
      Action: action,
      Remark: remark,
    });
...
const auditReject = async (id) => {
  try {
    const { value } = await ElMessageBox.prompt("可填写拒绝原因（选填）", "拒绝删除申请", {
