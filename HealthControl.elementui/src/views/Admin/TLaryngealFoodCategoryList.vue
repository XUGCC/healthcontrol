        ...
        <el-card class="box-card">
            ...
            <div class="margin-top-sm">
                <el-form :inline="true" :model="searchForm" size="default">
                    <el-form-item label="分类名称：如润喉类/辛辣类" prop="CategoryName">
                        <el-input v-model.trim="searchForm.CategoryName"  placeholder="请输入分类名称：如润喉类/辛辣类"  :clearable="true" ></el-input>
                    </el-form-item>
                    <el-form-item label="分类描述" prop="CategoryDesc">
                        <el-input v-model.trim="searchForm.CategoryDesc"  placeholder="请输入分类描述"  :clearable="true" ></el-input>
 							 ...
 							 <el-form-item label="分类类型：0=友好，1=忌口" class="search-input">
                 		 <el-select v-model="searchForm.CategoryType" class="search-input" :clearable="true" placeholder="请选择分类类型：0=友好，1=忌口" >
...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改喉部食物库分类' : '添加喉部食物库分类'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
            <el-form v-if="editorShow" ref="editModalForm" :rules="editModalFormRules" :model="formData" label-width="140px"
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="分类名称：如润喉类/辛辣类" prop="CategoryName">
                            <el-input type="text" v-model="formData.CategoryName"  placeholder="请输入分类名称：如润喉类/辛辣类"     :clearable="true"></el-input>
                        </el-form-item>
                    </el-col>


                    <el-col :span="24">
                        <el-form-item label="分类类型：0=友好，1=忌口" prop="CategoryType">
                       		  <el-switch v-model="formData.CategoryType"  :active-value="true" :inactive-value="false">
                            </el-switch>                          
                        </el-form-item>
                    </el-col>
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/TLaryngealFoodCategory/List" :column="columnList" :where="where">
            <template v-slot:header>
                ...
                <ExportButton exportUrl="/TLaryngealFoodCategory/Export" :where="searchForm"></ExportButton>        
            </template>
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/TLaryngealFoodCategory/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/TLaryngealFoodCategory/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/TLaryngealFoodCategory/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/TLaryngealFoodCategory/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
