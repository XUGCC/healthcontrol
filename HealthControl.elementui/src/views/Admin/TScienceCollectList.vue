        ...
        <el-card class="box-card">
            ...
            <div class="margin-top-sm">
                <el-form :inline="true" :model="searchForm" size="default">
                    ...
                    <el-form-item label="取消收藏时间" class="search-input">
                        <el-date-picker v-model="searchForm.UpdateTimeRange" type="datetimerange" start-placeholder="开始取消收藏时间"
                            end-placeholder="结束取消收藏时间" value-format="YYYY-MM-DD HH:mm:ss" >
                        </el-date-picker>                                       
                    </el-form-item>                
                    <el-form-item label="收藏操作时间" class="search-input">
                        <el-date-picker v-model="searchForm.CollectTimeRange" type="datetimerange" start-placeholder="开始收藏操作时间"
                            end-placeholder="结束收藏操作时间" value-format="YYYY-MM-DD HH:mm:ss" >
                        </el-date-picker>                                       
                    </el-form-item>                
                    <el-form-item label="取消收藏操作时间" class="search-input">
                        <el-date-picker v-model="searchForm.CancelCollectTimeRange" type="datetimerange" start-placeholder="开始取消收藏操作时间"
                            end-placeholder="结束取消收藏操作时间" value-format="YYYY-MM-DD HH:mm:ss" >
                        </el-date-picker>                                       
                    </el-form-item>                
...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改科普收藏' : '添加科普收藏'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
            <el-form v-if="editorShow" ref="editModalForm" :rules="editModalFormRules" :model="formData" label-width="140px"
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="取消收藏时间" prop="UpdateTime">
                    ...
                    <el-col :span="24">
                        <el-form-item label="收藏操作时间" prop="CollectTime">
                            <el-date-picker v-model="formData.CollectTime" align="right" type="datetime" placeholder="选择时间" value-format="YYYY-MM-DD HH:mm:ss">
                            </el-date-picker>
                        </el-form-item>
                    </el-col>

                    <el-col :span="24">
                        <el-form-item label="取消收藏操作时间" prop="CancelCollectTime">
                            <el-date-picker v-model="formData.CancelCollectTime" align="right" type="datetime" placeholder="选择时间" value-format="YYYY-MM-DD HH:mm:ss">
                            </el-date-picker>
                        </el-form-item>
                    </el-col>
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/TScienceCollect/List" :column="columnList" :where="where">
            <template v-slot:header>
                ...
                <ExportButton exportUrl="/TScienceCollect/Export" :where="searchForm"></ExportButton>        
            </template>
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/TScienceCollect/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/TScienceCollect/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/TScienceCollect/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/TScienceCollect/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
