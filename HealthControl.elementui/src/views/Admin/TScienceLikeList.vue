...
<!-- 编辑对话框 -->
        <el-dialog :title="formData.Id ? '修改科普点赞' : '添加科普点赞'" v-model="editorShow" width="50%" :lock-scroll="true"
            min-height="500px">
            <el-form v-if="editorShow" ref="editModalForm" :rules="editModalFormRules" :model="formData" label-width="140px"
              ...
              <el-row :gutter="10" class="edit-from-body">
                    ...
                    <el-col :span="24">
                        <el-form-item label="关联用户表主键" prop="UserId">
        ...
        <!-- 数据表格 -->
        <PaginationTable ref="PaginationTableId" url="/TScienceLike/List" :column="columnList" :where="where">
            <template v-slot:header>
                ...
                <ExportButton exportUrl="/TScienceLike/Export" :where="searchForm"></ExportButton>        
            </template>
...
const ShowEditModal = async (Id) => {
    ...
    const { Data } = await Post(`/TScienceLike/Get`, { Id: Id });
...
const CreateOrEditForm = async () => {
    ...
    await editModalForm.value.validate(async valid => {
        if (valid) {
            const { Success } = await Post(`/TScienceLike/CreateOrEdit`, formData);
...
const ShowDeleteModal = async (Id) => {
    ...
    try {
        ...
        const { Success } = await Post(`/TScienceLike/Delete`, { Id: Id });
        if (Success) {
            PaginationTableId.value.Reload(searchForm);
...
const BatchDelete = async () => {
    ...
    if (ids.length > 0) {
        try {
            ...
            const { Success } = await Post(`/TScienceLike/BatchDelete`, { Ids: ids });
            if (Success) {
                PaginationTableId.value.Reload(searchForm);
                ElMessage.success('删除成功');
            }
