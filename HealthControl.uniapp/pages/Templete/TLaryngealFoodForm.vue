<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="喉部食物库信息" />

        <uni-section title="请填写喉部食物库信息">
            <view class="form-container">
                ...
                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    ...
                    <uni-forms-item label="分类ID" required name="CategoryId">
                        <sigle-select url="/TLaryngealFoodCategory/List" columnName="CategoryName" columnValue="Id" v-model="formData.CategoryId">
                        </sigle-select>
                    </uni-forms-item>
                    <uni-forms-item label="食物名称" required name="FoodName">
                        <uni-easyinput v-model="formData.FoodName" placeholder="请输入食物名称" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="食物别名" required name="FoodAlias">
                        <uni-easyinput v-model="formData.FoodAlias" placeholder="请输入食物别名" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="功效危害说明" required name="EffectHarmDesc">
                        <uni-easyinput v-model="formData.EffectHarmDesc" placeholder="请输入功效危害说明" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="忌口建议" required name="SuggestContent">
                        <uni-easyinput v-model="formData.SuggestContent" placeholder="请输入忌口建议" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="食物配图URL" required name="PicUrl">
                        <upload-files v-model="formData.PicUrl"></upload-files>
                    </uni-forms-item>
        ...
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    ...
                    let { Data, Success } = await Post("/TLaryngealFood/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
