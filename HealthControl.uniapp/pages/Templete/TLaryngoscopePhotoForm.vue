<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="喉镜照片记录信息" />

        <uni-section title="请填写喉镜照片记录信息">
            <view class="form-container">

                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    label-position='top'>
                    <uni-forms-item label="审核更新时间" required name="UpdateTime">
                    ...
                    <uni-forms-item label="关联音频自查记录表主键t" required name="DetectId">
                        <sigle-select url="/TAudioScreenRecord/List" columnName="AudioUrl" columnValue="Id" v-model="formData.DetectId">
                        </sigle-select>
                    </uni-forms-item>
                    <uni-forms-item label="喉镜照片URL" required name="LaryngoscopePhotoUrl">
                        <upload-images v-model="formData.LaryngoscopePhotoUrl">
                        </upload-images>
                    </uni-forms-item>
                    <uni-forms-item label="照片描述" required name="PhotoDesc">
                        <upload-images v-model="formData.PhotoDesc">
                        </upload-images>
                    </uni-forms-item>
                    <uni-forms-item label="照片上传时间" required name="UploadTime">
                        <uni-datetime-picker type="datetime" return-type="string" v-model="formData.UploadTime"
                            primaryColor="var(--primary-color)" />
                    </uni-forms-item>
        ...
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    ...
                    let { Data, Success } = await Post("/TLaryngoscopePhoto/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
