<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="模型优化标注信息" />

        <uni-section title="请填写模型优化标注信息">
            <view class="form-container">

                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    label-position='top'>
                    <uni-forms-item label="授权状态更新时间" required name="UpdateTime">
                    ...
                    <uni-forms-item label="医院确诊结果" required name="HospitalDiagnoseResult">
                        <uni-easyinput v-model="formData.HospitalDiagnoseResult" placeholder="请输入医院确诊结果" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="标注类型" required name="LabelType">
                        <uni-data-checkbox v-model="formData.LabelType" selectedColor="var(--primary-color)"
                            :localdata="[{ text: '是', value: true }, { text: '否', value: false }]" />
                    </uni-forms-item>
                    <uni-forms-item label="标注说明" required name="LabelDesc">
                        <uni-easyinput v-model="formData.LabelDesc" placeholder="请输入标注说明" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="授权状态" required name="AuthStatus">
                        <uni-data-checkbox v-model="formData.AuthStatus" selectedColor="var(--primary-color)"
                            :localdata="[{ text: '是', value: true }, { text: '否', value: false }]" />
                    </uni-forms-item>
        ...
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    ...
                    let { Data, Success } = await Post("/TModelOptimizeLabel/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
