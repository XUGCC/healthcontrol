<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="风险评估问卷信息" />

        <uni-section title="请填写风险评估问卷信息">
            <view class="form-container">
                ...
                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    ...
                    <uni-forms-item label="问卷标题" required name="QuestionnaireTitle">
                        <uni-easyinput v-model="formData.QuestionnaireTitle" placeholder="请输入问卷标题" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="问卷描述" required name="QuestionnaireDesc">
                        <uni-easyinput v-model="formData.QuestionnaireDesc" placeholder="请输入问卷描述" primaryColor="var(--primary-color)" />
  	                 </uni-forms-item>
                    <uni-forms-item label="问题数量" required name="QuestionCount">
                        <uni-number-box :min="0" :max="99999" v-model="formData.QuestionCount" />
                    </uni-forms-item>
        ...
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    ...
                    let { Data, Success } = await Post("/TRiskAssessmentQuestionnaire/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
