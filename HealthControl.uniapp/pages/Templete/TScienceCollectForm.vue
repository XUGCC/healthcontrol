<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="科普收藏信息" />

        <uni-section title="请填写科普收藏信息">
            <view class="form-container">

                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    label-position='top'>
                    <uni-forms-item label="取消收藏时间" required name="UpdateTime">
                    ...
                    <uni-forms-item label="收藏操作时间" required name="CollectTime">
                        <uni-datetime-picker type="datetime" return-type="string" v-model="formData.CollectTime"
                            primaryColor="var(--primary-color)" />
                    </uni-forms-item>
                    <uni-forms-item label="取消收藏操作时间" required name="CancelCollectTime">
                        <uni-datetime-picker type="datetime" return-type="string" v-model="formData.CancelCollectTime"
                            primaryColor="var(--primary-color)" />
                    </uni-forms-item>
        ...
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    ...
                    let { Data, Success } = await Post("/TScienceCollect/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
