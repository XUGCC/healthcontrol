<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="自查报告导出记录信息" />

        <uni-section title="请填写自查报告导出记录信息">
            <view class="form-container">
                ...
                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    ...
                    <uni-forms-item label="报告导出时间" required name="ExportTime">
                        <uni-datetime-picker type="datetime" return-type="string" v-model="formData.ExportTime"
                            primaryColor="var(--primary-color)" />
                    </uni-forms-item>
                    <uni-forms-item label="导出格式" required name="ExportFormat">
                        <uni-data-checkbox v-model="formData.ExportFormat" selectedColor="var(--primary-color)"
                            :localdata="[{ text: '是', value: true }, { text: '否', value: false }]" />
                    </uni-forms-item>
                    <uni-forms-item label="导出文件URL" required name="ExportFileUrl">
                        <upload-files v-model="formData.ExportFileUrl"></upload-files>
                    </uni-forms-item>
                    <uni-forms-item label="导出用途" required name="ExportPurpose">
                        <uni-data-checkbox v-model="formData.ExportPurpose" selectedColor="var(--primary-color)"
                            :localdata="[{ text: '是', value: true }, { text: '否', value: false }]" />
                    </uni-forms-item>
        ...
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    ...
                    let { Data, Success } = await Post("/TScreenReportExportRecord/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
