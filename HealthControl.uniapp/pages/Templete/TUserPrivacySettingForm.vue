<template>
    <view>
        <uni-nav-bar dark :fixed="true" shadow background-color="var(--primary-color)" status-bar left-icon="left"
            left-text="返回" @clickLeft="goBack" title="用户隐私设置信息" />

        <uni-section title="请填写用户隐私设置信息">
            <view class="form-container">
                ...
                <uni-forms ref="editModalForm" :model="formData" :rules="editModalFormRules" label-width='100'
                    ...
                    <uni-forms-item label="本地存储开启状态：0=否，1=是" required name="LocalStorageStatus">
                        <uni-data-checkbox v-model="formData.LocalStorageStatus" selectedColor="var(--primary-color)"
                            :localdata="[{ text: '是', value: true }, { text: '否', value: false }]" />
                    </uni-forms-item>
                    <uni-forms-item label="数据匿名授权状态：0=否，1=是" required name="DataAnonymousAuth">
                        <uni-data-checkbox v-model="formData.DataAnonymousAuth" selectedColor="var(--primary-color)"
                            :localdata="[{ text: '是', value: true }, { text: '否', value: false }]" />
                    </uni-forms-item>
                    <uni-forms-item label="隐私协议同意状态：0=否，1=是" required name="PrivacyAgreeStatus">
                        <uni-data-checkbox v-model="formData.PrivacyAgreeStatus" selectedColor="var(--primary-color)"
                            :localdata="[{ text: '是', value: true }, { text: '否', value: false }]" />
                    </uni-forms-item>
        ...
        uni.showModal({
            ...
            success: async (res) => {
                if (res.confirm) {                                 
                    ...
                    let { Data, Success } = await Post("/TUserPrivacySetting/CreateOrEdit", formData);
                    if (Success) {
                        uni.showToast({
                            title: "操作成功",
                            icon: "success"
